...
- Specify the **constructor**: Look for the constructor function and note its parameters. Specify it using the `constructor` keyword and copy the constructor parameters. If the constructor is payable in the source code, mark it as `payable` in act (`constructor payable(<parameters>)`). Leave the `iff` blank for now.
...
10. **Reference the documentation**: Consult the [Storage, Typing and Expressions](./store_type.md), [Constructors](./constructors.md), and [Transitions](./transitions.md) sections for detailed explanations
